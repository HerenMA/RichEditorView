...
@property (nonatomic) IBOutlet UIView *shortcutView;
...
- (void)viewDidLoad
{
    ...
    CGRect shortcutViewFrame = _shortcutView.frame;

    shortcutViewFrame.origin.y = self.navigationController.navigationBar.frame.size.height + 40;
    _shortcutView.frame = shortcutViewFrame;

    CGRect squarePickerFrame = _squarePicker.frame;

    squarePickerFrame.origin.y += 10;
    _squarePicker.frame = squarePickerFrame;

    if (@available(iOS 11.0, *)) {
        CGRect barFrame = _barPicker.frame;
        barFrame.origin.y -= self.view.safeAreaInsets.bottom > 0 ? 30 : 0;
        _barPicker.frame = barFrame;
    }
